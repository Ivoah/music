...
def main(rawArgs: String*): Unit = {
  class Args(args: Seq[String]) extends ScallopConf(args) {
    ...
    val noPoller: ScallopOption[Boolean] = opt[Boolean](default = Some(false))
  ...
  if (args.noPoller.toOption.get) {
    println("Skipping poller")
  } else {
    println("Starting poller")
    Timer().scheduleAtFixedRate(new TimerTask {
      def run(): Unit = Try(spotify.nowPlaying()).toOption.flatten match {
